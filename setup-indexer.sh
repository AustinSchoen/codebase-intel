...
# These point at the central server's backends. The daemon connects to
# the MCP server via -server-url and doesn't talk to qdrant/postgres
# directly; these fields satisfy the indexer config schema but are
# unused in daemon mode.
vector_store:
  url: "$SERVER_URL"
